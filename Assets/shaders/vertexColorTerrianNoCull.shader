...
Shader "Custom/vertexColorTerrianNoCull" {
	Properties {
	...
	SubShader {
		...
		Pass {	
			...
			fixed4 frag(v2f i) : Color {
				//
	        	fixed4 col =  tex2D(_MainTex, i.uv)*((i.vertColor+UNITY_LIGHTMODEL_AMBIENT)/1.5+tex2D(_LightMap, (i.offPos.xz+float2(_CameraSize, _CameraSize))/(2*_CameraSize))*2 );
				return col;
			}	
