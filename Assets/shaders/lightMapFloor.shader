	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(v2f i) : Color {
	            ...
	            //return tex2D(_MainTex, i.uv)*((UNITY_LIGHTMODEL_AMBIENT)/1.5+tex2D(_LightMap, (i.offPos.xz+float2(_CameraSize, _CameraSize))/(2*_CameraSize))*2)*_Color;
	            ...
	            return col;
	            //*(tex2D(_LightMap, ((i.offPos.xz+float2(_CameraSize, _CameraSize))/(2*_CameraSize)))+UNITY_LIGHTMODEL_AMBIENT)*2*_Color;
