Shader "Custom/waterfall_short" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_Mask1 ("Base (RGB)", 2D) = "white" {}
		_UVAnimX ("UV Anim X", float) = 0
		_UVAnimY ("UV Anim Y", float) = 0

		_MainTex2 ("Base (RGB)", 2D) = "white" {}
		_Mask2 ("Base (RGB)", 2D) = "white" {}
	...
	SubShader {
		...
		*/
		...
		Pass {
			...
			fixed4 frag(v2f i) : Color {
                fixed4 col;
                fixed4 baseCol = tex2D(_MainTex, i.uv1);
                fixed4 maskCol = tex2D(_Mask1, i.uv);
                col = maskCol*baseCol*i.color;
				return col;
			}	
	        ENDCG
		}
